...
import {historyColumns, deltaColumn} from 'https://deno.land/x/pretty_benching@v0.3.0/history_extensions.ts';
import {prettyBenchmarkHistory} from 'https://deno.land/x/pretty_benching@v0.3.0/pretty_benchmark_history.ts';
...
import type { BenchIndicator, Thresholds } from "https://deno.land/x/pretty_benching@v0.3.0/types.ts";
...
let historicData;
try {
  historicData = JSON.parse(
    Deno.readTextFileSync("./benchmarks/benchmark_history.json"),
  );
} catch (e) {
  console.warn(`⚠ cant read history file. (${e.message})`);
}
const history = new prettyBenchmarkHistory(historicData, {strict: true, runExtras: ()=> Deno.version});
...
runBenchmarks(
  ...
  .then(prettyBenchmarkDown(
    (markdown) => { markdownString = markdown },
    {
      ...
      groups: [
        ...
        },
        {
          ...
          description:
            "There are also predefined columns for you to use with `prettyBenchmarkHistory`. There is `historyColumns`, which gives you each historic run's result as a separate column, and `deltaColumn`, which shows you how much the results changed from the previous run for each benchmark.",
          columns: [
            ...defaultColumns(["name", "runsCount"]),
            ...historyColumns(history),
            {title: "Current", align: "right", propertyKey: "measuredRunsAvgMs", toFixed: 4},
            deltaColumn(history)
          ],
        },
      ],
    ...
    if(Deno.args.indexOf("--save-history") !== -1) {
      const id = Deno.args.find(a => a.startsWith("--id="))?.replace("--id=", "");
      console.log(`Saving history${id ? ` with id: ${id}`: ""}`);
      Deno.writeTextFileSync("./benchmarks/benchmark_history.json", history.addResults(results, {id}).getDataString());
    }
    if(Deno.args.indexOf("--save-markdown") !== -1) {
      console.log("Saving markdown");
      Deno.writeTextFileSync("./benchmarks/pr_benchmark.md", markdownString);
    }
